...
exports.parseBatch = function(text){
  ...
  // parse timestamp
  collectedAt = moment(collectedAt, moment.ISO_8601);

  // ignores if year is 2004 and set current Brasilia time
  if (collectedAt.year() <= 2004) {
    collectedAt = moment().tz("Brazil/East");
  }

  // ignore timezone
  collectedAt = collectedAt.format('YYYY-MM-DDTHH:mm:ss') + 'Z';
